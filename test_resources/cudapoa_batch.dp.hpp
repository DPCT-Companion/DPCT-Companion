        ...
        , cudaaligner_band_width_(cudaaligner_band_width)
{
    ...
    for(int32_t dev_id = 0; dev_id < num_devices_; dev_id++)
    {
        ...
        */
        GW_CU_CHECK_ERR((dpct::dev_mgr::instance().select_device(dev_id), 0));
        /*
        DPCT1003:89: Migrated API does not return error code. (*, 0) is
        inserted. You may need to rewrite this code.
        */
        GW_CU_CHECK_ERR((sycl::free(0, dpct::get_default_queue()), 0));
    }
...
CUDAPolisher::~CUDAPolisher()
{
    ...
    /*
    DPCT1007:90: Migration of this CUDA API is not supported by the Intel(R)
...
void CUDAPolisher::find_overlap_breaking_points(std::vector<std::unique_ptr<Overlap>>& overlaps)
{
    if (cudaaligner_batches_ >= 1)
    {
        ...
        for(int32_t device = 0; device < num_devices_; device++)
        {
            ...
            */
            GW_CU_CHECK_ERR(
                (dpct::dev_mgr::instance().select_device(device), 0));
            ...
            */
            /*
            DPCT1072:93: DPC++ currently does not support getting the available
            memory on the current device. You may need to adjust the code.
            */
            GW_CU_CHECK_ERR((total = dpct::get_current_device()
                                         .get_device_info()
                                         .get_global_mem_size(),
                             0));
    ...
    bool drop_unpolished_sequences)
{
    ...
    else
    {
        ...
        for(int32_t device = 0; device < num_devices_; device++)
        {
            ...
            */
            /*
            DPCT1072:96: DPC++ currently does not support getting the available
            memory on the current device. You may need to adjust the code.
            */
            GW_CU_CHECK_ERR((total = dpct::get_current_device()
                                         .get_device_info()
