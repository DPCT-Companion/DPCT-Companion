                                    ...
                                    sycl::nd_item<3> item_ct1)
{
    ...
    */
    const WordType x = sycl::shift_group_right(item_ct1.get_sub_group(),
                                               v >> (word_size - 1), 1);
                                     ...
                                     sycl::nd_item<3> item_ct1)
{
    ...
    */
    const WordType x = sycl::shift_group_left(item_ct1.get_sub_group(),
                                              v << (word_size - 1), 1);
             ...
             idx += warp_size)
        {
            if (idx < n_words)
            {
                ...
                if (warp_mask == 0xffff'ffffu &&
                    ...
                    /*
                    DPCT1023:69: The DPC++ sub-group does not support mask
                    options for sycl::shift_group_left.
                    */
                    warp_carry = sycl::shift_group_left(
                        item_ct1.get_sub_group(), warp_carry, warp_size - 1);
                if (item_ct1.get_local_id(2) != 0)
                    warp_carry = 0;
                pv(idx, t) = pv_local;
                mv(idx, t) = mv_local;
            ...
            if (idx < n_words)
            {
                ...
                if (warp_mask == 0xffff'ffffu &&
                    (item_ct1.get_local_id(2) == 0 ||
                     item_ct1.get_local_id(2) == 31))
                    /*
                    DPCT1023:70: The DPC++ sub-group does not support mask
                    options for sycl::shift_group_left.
                    */
                    warp_carry = sycl::shift_group_left(
                        item_ct1.get_sub_group(), warp_carry, warp_size - 1);
            ...
            if (idx < n_words_band)
            {
                ...
                if (warp_mask == 0xffff'ffffu &&
                    (item_ct1.get_local_id(2) == 0 ||
                     item_ct1.get_local_id(2) == 31))
                    /*
                    DPCT1023:71: The DPC++ sub-group does not support mask
                    options for sycl::shift_group_left.
                    */
                    carry =
                        sycl::shift_group_left(item_ct1.get_sub_group(),
                                               delta_right.y(), warp_size - 1);
                if (item_ct1.get_local_id(2) != 0)
                    carry = 0;
...
int32_t myers_compute_edit_distance(std::string const& target, std::string const& query)
{
    ...
    */
    GW_CU_CHECK_ERR((stream = dpct::get_current_device().create_queue(), 0));
    {
        ...
        */
        GW_CU_CHECK_ERR((stream->memcpy(sequences_d.data(), query.data(),
                                        sizeof(char) * get_size(query)),
                         0));
        /*
        ...
        */
        GW_CU_CHECK_ERR(
            (stream->memcpy(sequences_d.data() + max_sequence_length,
                            target.data(), sizeof(char) * get_size(target)),
             0));
        /*
        DPCT1003:75: Migrated API does not return error code. (*, 0) is
        inserted. You may need to rewrite this code.
        */
        GW_CU_CHECK_ERR((stream->memcpy(sequence_lengths_d.data(),
                                        lengths.data(), sizeof(int32_t) * 2),
                         0));
    ...
    You may need to rewrite this code.
    */
    GW_CU_CHECK_ERR((dpct::get_current_device().destroy_queue(stream), 0));
        ...
        */
        ...
        });
        {
            ...
            n_blocks[1] = ceiling_divide<int32_t>(get_size<int32_t>(target) + 1,
                                                  n_threads[1]);
            /*
            DPCT1049:82: The workgroup size passed to the SYCL kernel may exceed
...
{
    {
        ...
        /*
        DPCT1049:85: The workgroup size passed to the SYCL kernel may exceed the
    ...
    }
    {
        ...
        /*
        DPCT1049:86: The workgroup size passed to the SYCL kernel may exceed the
        limit. To get the device limit, query info::device::max_work_group_size.
        Adjust the workgroup size if needed.
        */
        stream->submit([&](sycl::handler &cgh) {
                      ...
                      sycl::queue *stream)
{
    ...
    const sycl::range<3> blocks(
        1, ceiling_divide<int32_t>(n_alignments, threads[1]), 1);
    /*
    DPCT1049:87: The workgroup size passed to the SYCL kernel may exceed the
    limit. To get the device limit, query info::device::max_work_group_size.
    Adjust the workgroup size if needed.
    */
    stream->submit([&](sycl::handler &cgh) {
